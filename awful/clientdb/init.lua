...
local module = {}
module.db = awful.util.getdir("config").."/rules.db"

-- Check if required module loaded
if not has_DBI then
    naughty.notify({title = "ClientDB", text = "There is an error loading Lua DBI module", timeout = 5})
end
...
function module.save(c)
    ...
    local function optimize(obj)
        if type(obj) == "string" then return obj -- XXX: string.gsub(obj, "[^a-zA-Z0-9-.\/=:](),", "") ~
    ...
    sth:close()
    dbh:close()
    naughty.notify({title = "ClientDB", text = "Client saved", timeout = 5})
end
