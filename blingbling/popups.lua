  ...
  awful.button({ }, 5, function()
   ...
   awful.button({ }, 1, function()
    if args["terminal"] then
      awful.spawn.easy_async_with_shell(args["terminal"] .. " -e htop")
    else
      awful.spawn.easy_async_with_shell("xterm" .. " -e htop")
    end
  end)
  ))
end
...
local function get_netinfo( my_title_color, my_established_color, my_listen_color)
  local str=awful.util.pread('/usr/bin/netstat -pa -u -t | grep -v TIME_WAIT')
