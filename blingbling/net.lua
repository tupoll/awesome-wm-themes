...
local function get_net_infos(n_graph)
  ...
  for line in io.lines("/compat/linux/proc/net/dev") do
    ...
    -- Transmited bytes, 7 fields from end of the line
      ...
      local state
      for line in io.lines("/compat/linux/proc/net/dev"..string.match(line, "^[%s]?[%s]?[%s]?[%s]?([%w]+):")) do
        state = line
      end
      ...
      if data[n_graph][device.."_state"] == "up" then
        for line in io.lines("/compat/linux/proc/net/dev"..string.match(line,
             "([%d]+)%s+%d+%s+%d+%s+%d+%s+%d+%s+%d+%s+%d+%s+%d$")) do
          carrier = line
        end
