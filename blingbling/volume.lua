...
local function set_master(parameters)
    local cmd = "mixer vol " ..parameters
    local f=io.popen(cmd)
    f:close()
end
local function update_mixer(volume_graph)
    local state
    local value
    data[volume_graph].mixertimer = timer({timeout = 0.5})
    data[volume_graph].mixertimer:connect_signal("timeout", function() 
      data[volume_graph].state, value = mixer(); set_value(volume_graph,value/100) 
    end)
    data[volume_graph].mixertimer:start()
    return volume_graph
end
...
function set_master_control(volume_graph)
    ...
    awful.button({ }, 1, function()
      set_mixer("mixer vol 0")
    end),
    awful.button({ }, 5, function()
      set_mixer("mixer vol -2")
    end),
...
function volume.new(args)
    ...
    for _, prop in ipairs(properties) do
        volume_graph["vol+_" .. prop] = volume["vol+_" .. prop]
    end
